...
router.get('/temp-correct-match-37', async (req, res) => {
    try {
        // 1. Guardar posiciones actuales como anteriores antes de recalcular
        await guardarPosicionesActualesComoAnteriores();

        // 2. Actualizar resultados_reales para España vs Arabia (ID 37) a 4-0
        await query(
            `INSERT INTO resultados_reales (partido_id, goles_local, goles_visitante) VALUES ($1, $2, $3)
             ON CONFLICT (partido_id) DO UPDATE SET goles_local=$2, goles_visitante=$3`,
            [37, 4, 0]
        ...
        // 3. Actualizar resultados_pendientes
        await query(
            `UPDATE resultados_pendientes 
             SET goles_local=4, goles_visitante=0, validado=TRUE, fecha_validacion=NOW()
             WHERE partido_id=37`
        );

        // 4. Enviar correos de resultado a todos los participantes con pronóstico
        const pros = await query(
            `SELECT p.id_usuario, p.goles_local AS pro_local, p.goles_visitante AS pro_visitante,
                    u.nombre, u.correo
             FROM pronosticos p INNER JOIN usuarios u ON p.id_usuario=u.id_usuario
             WHERE p.partido_id=37 AND u.correo IS NOT NULL AND u.correo!=''`
        );

        for (const pro of pros.rows) {
            let puntos = 0, estado = 'Falló';
            if (pro.pro_local===4 && pro.pro_visitante===0) { 
                puntos=5; 
                estado='Exacto'; 
            }
            else if ((pro.pro_local>pro.pro_visitante && 4>0) || (pro.pro_local<pro.pro_visitante && 4<0)) { 
                puntos=3; 
                estado='Acierto'; 
            }
        ...
        // 5. Recalcular puntos generales de todos los usuarios
        const prosCalc = await query(
            `SELECT p.id_usuario, p.goles_local AS pro_local, p.goles_visitante AS pro_visitante,
                    r.goles_local AS real_local, r.goles_visitante AS real_visitante
             FROM pronosticos p INNER JOIN resultados_reales r ON p.partido_id=r.partido_id`
        );
        ...
        return res.json({ ok: true, message: 'España vs Arabia Saudita corregido a 4-0 exitosamente, correos enviados y puntos generales recalculados.' });
