...
const allowedOrigins = [
    "http://127.0.0.1:5500",
    "http://localhost:5500",
    "http://localhost",           // ← AGREGA ESTA
    "http://localhost:80",        // ← Y ESTA
    ...
    try {
        ...
        if (!nombre || !correo || !password || !preguntaSeguridad || !respuestaSeguridad || !codigoInvitacion) {
            return res.status(400).json({
                ok: false,
                message: "Todos los campos son obligatorios, incluyendo el código de invitación."
            });
        }

        const pool = await poolPromise;

        // ✅ Validar código de invitación contra la BD
        const codResult = await pool.request()
            .input("Codigo", sql.NVarChar(100), codigoInvitacion.trim())
            .query(`
                SELECT IdCodigo, Utilizado
                FROM dbo.CodigosInvitacion
        ...
        if (codResult.recordset[0].Utilizado === true) {
        ...
        await pool.request()
            ...
            .query(`
                ...
                SET Utilizado = 1, FechaUso = GETDATE(), IdUsuario = @IdUsuario
...
app.post("/api/actualizar-perfil", async (req, res) => {
    try {
        ...
        if (nombresReservados.map(normalizar).includes(normalizar(nuevoNombre))) {
...
// require('./sync-resultados'); // ← comenta esto hasta el 11 de Junio
